...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchStories = async () => {
      try {
        const response = await fetch(`https://backend-blog-7.onrender.com/api/stories`, {
          headers: {
          ...
          <div className="profile-stories">
            {stories.map((story) => (
              <div key={story._id} className="story-item">
                ...
                {story.image && (
                  <img
                    src={`https://backend-blog-7.onrender.com/${story.image}`} // Adjust the image path based on where the images are stored
                    alt={story.title}
